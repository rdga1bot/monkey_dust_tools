...
Collision: дослідити що Kenshi реально використовує для колізії пропсів
(окремі collision-меші vs візуальний меш — інвентаризувати `_collis`/
`_COL`/`.bin` файли, вже помічені в Фазі 3 manifest, не вгадувати) —
або конвертувати окремі collision-меші тим самим конвеєром із суфіксом,
або задокументувати пропозицію (напр. "collision = спрощений LOD2/3
меш у Jolt MeshShape") для НАСТУПНОГО (instancing) проходу. НЕ
реалізовувати нову фізику в цій фазі.
