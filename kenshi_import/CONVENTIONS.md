...
## Наступний крок (Фаза 4)

gltfpack: перевірити підтримку `KHR_mesh_quantization` рушієм (тест
на одному меші) + перевірити наявність `EXT_meshopt_compression`
декодера (якщо немає — не вмикати `-c`/`-cc`). LOD-ланцюги
(LOD0=100%, LOD1 `-si 0.5`, LOD2 `-si 0.2`, LOD3 `-si 0.05`).
**STOP-GATE**: якщо рушій не підтримує потрібні gltfpack-розширення —
зупинитись і повідомити користувача, не продовжувати мовчки.
