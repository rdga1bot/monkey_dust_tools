...
## Фаза 4 — LOD-ланцюги (ЗАВЕРШЕНО 2026-08-02, після engine-фіксів)

**gltfpack встановлено через npm** (`npm install -g gltfpack`, за вибором
користувача — AUR/flatpak відкинуті: flatpak не мав пакету, AUR вимагав
makepkg-збірку третьосторонніх джерел). `/home/rdga1/.local/bin/gltfpack`
(gltfpack 1.2).

**`tools/kenshi_import/gltfpack_lods.py`** — новий скрипт: для кожного
GLB у вхідній директорії генерує 4 файли: `<name>.lod0.glb` (лише
квантизація, без спрощення), `.lod1.glb` (`-si 0.5`), `.lod2.glb`
(`-si 0.2`), `.lod3.glb` (`-si 0.05`) — точні співвідношення з
master-плану, БЕЗ індивідуального тюнінгу під кожен меш (задокументовано
в docstring файлу — LOD1-3 не перевірені візуально, лише LOD0). JSON
summary report (`--report`) з trailer/розмір на кожен LOD.

**Реальний прогін на всьому "moor" building-kit** (ті самі 200 мешів
Фази 3): **200/200 (100%) успіх**, gltfpack-помилок 0.
`phase4_lods_report.json` (сирі дані):

| LOD  | Σ трикутників | Σ розмір | середній розмір/меш |
|------|---------------|----------|----------------------|
| lod0 | 192,399       | 21.04MB  | 105.2KB              |
| lod1 | 151,553       | 20.25MB  | 101.3KB              |
| lod2 | 147,411       | 20.20MB  | 101.0KB              |
| lod3 | 147,120       | 20.19MB  | 101.0KB              |

Джерело (`.mesh`, до конвертації): 28.04MB. **Чесний висновок**:
квантизація (LOD0) дає основний виграш розміру (28.04MB→21.04MB,
~25%); подальше спрощення (`-si` LOD1→LOD3) ріже трикутники ще на
~23%, але розмір буферів padає лише ще на ~4% сумарно — на цьому
конкретному наборі (прості модульні деталі будівель, вже близькі до
геометричного мінімуму, `-se` default 0.01 error limit не дозволяє
спрощувати далі). Це РЕАЛЬНА метрика для цього набору, НЕ
екстраполюється на меші з великою кількістю трикутників (дерева/скелі
з detail-геометрією матимуть інший профіль).

**LOD0 геометрична парність, живо перевірено** (`phase4_lod0_parity.png`):
`moorstairs.glb` (Фаза 3, без gltfpack) vs `moorstairs.lod0.glb` —
ідентична східчаста силует-форма, ідентична кількість індексів (891
трикутники в обох), вершини дедуплікуються gltfpack (358→330, звичайна
lossless оптимізація, не втрата геометрії). Підтверджує: quantization
(вже виправлена в `PropMesh`) зберігає форму меша коректно на РЕАЛЬНОМУ
Kenshi-мешi, не лише на тестовому `wall.mesh`.

**Відкрите, чесно незадокументоване до кінця спостереження:** на цьому
ж скріншоті два об'єкти показали РІЗНЕ затінення (один явно темніший/
чорний, інший освітлений тан-колір) попри ІДЕНТИЧНІ дані матеріалу
(перевірено напряму з JSON GLB-чанку: обидва `StaticObject`,
`baseColorFactor=[0.8,0.8,0.8,1]`, без текстури). Корінь НЕ знайдено
в межах цієї сесії (не критично для LOD-валідації — геометрія
підтверджена коректною незалежно від цього); залишено як відкритий
пункт для майбутньої сесії, не замасковано.

## Фаза 5 — Collision, дослідження (ЗАВЕРШЕНО 2026-08-02, без нового коду)

**Реальний формат, знайдено читанням файлів (не вгадано):** Kenshi
авторить колізію ОКРЕМО від візуального `.mesh`, у форматі
**NVIDIA PhysX 2.8.5 NxuStream2 XML** (`<NXUSTREAM2><NxuPhysicsCollection
... sdkVersion="285">`) — повністю ІНША серіалізація, не OGRE,
конвертер `.mesh`→XML (`OgreXMLConverter`) цей формат не розуміє й
не може розпарсити.

**Покриття (у "moor" наборі, 200 мешів):** лише **9 реальних
collision-файлів** (6× `.xml`, 3× `.bin`, ~4.5% покриття) — переважна
більшість окремих деталей (essentials/floor/interior-варіанти) власної
колізії НЕ має; лише "структурні" деталі (roof, base, door) мають
авторовану колізію — інші, ймовірно, покладаються на колізію
структурної деталі, до якої прикріплені (не підтверджено напряму,
логічний висновок з даних).

**Реальна геометрія колізії (спот-перевірка 6 файлів):** **22 box-
shapes** (`NxBoxShapeDesc`, `dimensions="hx hy hz"` + `localPose`
= 3x3-ротація+translation, 12 чисел) проти лише **1 convex hull**
(`NxConvexMeshDesc`) і **0 triangle-mesh** колізій. Kenshi авторить
колізію будівель ЯК composed-box примітиви (дешево, точно для
прямокутних інтер'єрів), НЕ як спрощену копію візуального меша.

**Пропозиція для НАСТУПНОГО проходу (НЕ реалізовано цього разу):**
написати окремий NxuStream2 XML-парсер (`NxBoxShapeDesc`
dimensions+localPose → Jolt `BoxShape` + локальний transform;
`NxConvexMeshDesc` verts → Jolt `ConvexHullShape`) і компонувати в
Jolt `StaticCompoundShape` на будівлю. Це ТОЧНІШЕ й ДЕШЕВШЕ за
"colision = спрощений LOD2/3 mesh" (початкова ідея з master-плану) —
реальні дані показують, що Kenshi САМ уже авторить composed-box
примітиви, тож відтворення цього напряму дає кращий результат, ніж
апроксимація мешем. Для ~95.5% деталей БЕЗ власної колізії — потрібне
окреме рішення (напр. успадкування колізії батьківської структурної
деталі через `.zone`-ієрархію, поза обсягом цього дослідження).

## Наступний крок (Фаза 6)

Фінальний звіт (`PIPELINE_REPORT.md`, ≤50 рядків): числа конвертації,
memory-budget порівняння, LOD-таблиця, відомі обмеження (skeletal
меші відкладено, список зламаних мешів), one-line команда відтворення
per city. Оновити `CLAUDE.md` новою секцією "Asset pipeline".
